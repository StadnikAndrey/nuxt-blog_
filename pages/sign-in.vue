      ...
      <div class="login__content" v-if="user == null">
        ...
        <form class="login__form" autocomplete="off">
          <div class="login__form-row">
            ...
            <input class="login__form-input login__form-input-user" name="login" type="text" placeholder="Email"
              v-model="form.email" />
          </div>
          <div class="login__form-row">
            <label class="login__form-label">Password</label>
            <input class="login__form-input login__form-input-password" name="pass" type="password"
              placeholder="Password" v-model="form.password" />
          </div>

          <button class="login__btn" type="button" @click="sendFormLogin"
            @contextmenu.prevent="notificationsAdd('notification')">
            Sign in
          </button>
        </form>
        ...
        <div class="login__go-site">
          <NuxtLink class="login__go-site-link" to="/sign-up">
            Registration
          </NuxtLink>
        </div>
      </div>
...
export default {
  name: "SignIn",
  layout: "empty",
  mixins: [fingerprint, notifications],
  data() {
    return {
      form: {
        email: null,
  ...
  async mounted() {
    this.form.fingerprint = await this.fingerprint();
  },
  beforeDestroy() {
    this.setErrorsLogin([]);
  },
  computed: {
    ...mapGetters({ serverErrors: "user/getErrorsLogin" }),
    ...mapGetters({ user: "user/getUser" }),
  },
  methods: {
    ...mapActions({ login: "user/login" }),
    ...mapActions({ setErrorsLogin: "user/setErrorsLogin" }),
    async sendFormLogin() {
      let login = await this.login(this.form).catch((e) => {

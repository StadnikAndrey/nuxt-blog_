<template>
  <client-only>
    <div>
      <h3 class="article-add__title">Add article</h3>
      ...
      <div class="article-add__section">
        ...
        <button class="article-add__btn-create" type="button" @click="createArticle">
          Create article
        </button>
      </div>
  ...
  methods: {
    async createArticle() {
      ...
      let res = await this.$axios
        .$post("/api/insert-article", article)
        .catch((e) => {
          this.notificationsAdd(`Error insert article. ${e.response.status}`);
        });
